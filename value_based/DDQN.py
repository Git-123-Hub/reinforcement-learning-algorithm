...
class DDQN(DQN):
    def __init__(self, env, Q_net: Type[QNet], config):
    ...
    def get_next_state_value(self, next_states):
        ...
        best_action = self.Q(next_states).detach().argmax(1).unsqueeze(1)  # shape: [batch_size, action_dim]
        # calculate next_states_value using `self.target_Q` and the best action
        return self.target_Q(next_states).detach().gather(1, best_action).squeeze(1)  # shape: batch_size
